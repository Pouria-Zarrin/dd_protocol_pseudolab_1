...
def train_valid_test(file_name):
    ...
    output_dir = os.path.join(file_path, protein, f"iteration_{n_it}")
    os.makedirs(output_dir, exist_ok=True)

    train_set = open(os.path.join(output_dir, "train_set.txt"), 'a')
    test_set = open(os.path.join(output_dir, "test_set.txt"), 'a')
    valid_set = open(os.path.join(output_dir, "valid_set.txt"), 'a')
...
if __name__ == '__main__':
    ...
    f_names = []
    for f in glob.glob(data_directory+'/*.smi'):
        f_names.append(f)
    print("Number of .smi files found:", len(f_names))
